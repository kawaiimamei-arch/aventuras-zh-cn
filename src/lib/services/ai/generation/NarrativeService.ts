...
export class NarrativeService {
  ...
  private buildAdventurePriming(pov: string, tense: string, protagonistName: string): string {
    ...
    // Chinese language instruction used regardless of POV
    const chineseInstruction = `
IMPORTANT: Write ALL responses in Simplified Chinese (简体中文). All narration, descriptions, and NPC dialogue must be in Chinese. Only character names may remain in their original language.`
...
I am the player. You narrate the world around me. Begin when I take my first action.`
  ...
  private buildCreativeWritingPriming(pov: string, tense: string, protagonistName: string): string {
    ...
    const chineseInstruction = `
IMPORTANT: Write ALL responses in Simplified Chinese (简体中文). All prose, descriptions, and dialogue must be in Chinese. Only character names may remain in their original language.`
